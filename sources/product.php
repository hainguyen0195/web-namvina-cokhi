...
$select_detail = ' type, id, ten' . $lang . ', tenkhongdauvi, tenkhongdauen, mota' . $lang . ', noidung' . $lang . ', masp, luotxem, id_brand, id_mau, id_size, id_list, id_cat, id_item, id_sub, id_tags, photo, options, giakm, giamoi, gia,rate,info_more ';
if ($id != '') {
	/* Lấy sản phẩm detail */
	...
	$file_product = $d->rawQuery("select taptin,ten$lang from #_gallery where id_photo = ? and com='product' and type = ? and kind='man' and val = ? and hienthi > 0 order by stt,id desc", array($row_detail['id'], $type, 'taptin'));
