...
$chungnhan = $d->rawQuery("select ten$lang, photo, link from #_photo where type = ? and hienthi > 0 order by stt,id desc", array('chung-nhan'));
...
if (isset($_POST['submit-newsletter'])) {
    ...
    if (($scoreCaptcha >= 0.5 && $actionCaptcha == 'Newsletter') || $testCaptcha == true) {
        ...
        if ($d->insert('newsletter', $data)) {
            $func->transfer(dangkynhantintc, $config_base);
        } else {
            $func->transfer(dangkynhantintb, $config_base, false);
        }
    } else {
        $func->transfer(dangkynhantintb, $config_base, false);
    }
