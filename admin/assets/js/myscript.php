<script type="text/javascript">
	function createKeyInfo() {
		$('body').on("keyup", "#tenvi", function() {
			var slug = slugConvert($(this).val(), 'vi');
			$("#keyinfo").val(slug);
		});
	}

	function addKeyInfo() {
		var listinfo = $(".thongsosanpham").val();
		var idpro = $("#id_productthis").val();

		$.ajax({
			url: 'ajax/ajax_thongso.php',
			type: 'POST',
			dataType: 'html',
			async: false,
			data: {
	...
	function readImage(inputFile, elementPhoto) {
		...
		console.log("elementPhoto", elementPhoto);
	...
	$(document).ready(function() {
		...
		photoZone("#icon-zone", "#file-icon-zone", "#iconUpload-preview img");
		photoZone("#icon-zone2", "#file-icon-zone2", "#iconUpload-preview2 img");
		...
		createKeyInfo();
		$('body').on("change", ".thongsosanpham", function() {
			addKeyInfo();
		})
