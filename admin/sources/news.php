...
function save_item()
{
	...
	if ($id && $act != 'save_copy') {
		...
		if (isset($_FILES['icon'])) {
			...
			if ($icon = $func->uploadImage("icon", $config['news'][$type]['img_type_list'], UPLOAD_NEWS, $file_icon_name)) {
				$data['icon'] = $icon;
				$row = $d->rawQueryOne("select id, icon from #_news_list where id = ? and type = ? limit 0,1", array($id, $type));
