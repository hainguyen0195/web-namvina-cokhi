...
function save_item()
{
	...
	if ($data) {
		...
		if (isset($data['apdung']) && is_array($data['apdung'])) {
			$locations = $data['apdung'];
			$jsonLocations = json_encode($locations);
			$data['apdung']	= $jsonLocations;
		}
		...
		if (isset($config['product'][$type]['infoproduct']) && $config['product'][$type]['infoproduct'] == true) {
			if (isset($_POST['info_group']) && ($_POST['info_group'] != '')) $data['id_info'] = implode(",", $_POST['info_group']);
			else $data['id_info'] = "";
		}
	...
	if ($id && $act != 'save_copy') {
		...
		if (isset($_FILES['file2'])) {
			...
			if ($photo2 = $func->uploadImage("file2", $config['product'][$type]['img_type'], UPLOAD_PRODUCT, $file_name2)) {
				...
				if ($row['id']) $func->delete_file(UPLOAD_PRODUCT . $row['photo2']);
			}
		}
...
function save_list()
{
	...
	if ($id) {
		...
		if (isset($_FILES['icon'])) {
			$file_icon_name = $func->uploadName($_FILES['icon']["name"]);
			if ($icon = $func->uploadImage("icon", $config['product'][$type]['img_type_list'], UPLOAD_PRODUCT, $file_icon_name)) {
				$data['icon'] = $icon;
		...
		if (isset($_FILES['icon2'])) {
			$file_icon_name2 = $func->uploadName($_FILES['icon2']["name"]);
			if ($icon2 = $func->uploadImage("icon2", $config['product'][$type]['img_type_list'], UPLOAD_PRODUCT, $file_icon_name2)) {
				$data['icon2'] = $icon2;
				$row = $d->rawQueryOne("select id, icon2 from #_product_list where id = ? and type = ? limit 0,1", array($id, $type));
				if ($row['id']) $func->delete_file(UPLOAD_PRODUCT . $row['icon2']);
			}
		}
