...
function save_static()
{
	...
	if ($id) {
		...
		if (isset($_FILES['filemb'])) {
			$filemb_name = $func->uploadName($_FILES['filemb']["name"]);
			if ($photomb = $func->uploadImage("filemb", $config['photo']['photo_static'][$type]['img_type'], UPLOAD_PHOTO, $filemb_name)) {
				$data['photomb'] = $photomb;
				$row = $d->rawQueryOne("select id, photomb from #_photo where id = ? and act = ? and type = ? limit 0,1", array($id, 'photo_static', $type));
				if ($row['id']) $func->delete_file(UPLOAD_PHOTO . $row['photomb']);
			}
		}
			...
			for ($i = 0; $i < count($dataMultiTemp); $i++) {
				...
				if (isset($config['photo']['man_photo'][$type]['images_photo']) && $config['photo']['man_photo'][$type]['images_photo'] == true) {
					if (isset($_FILES["file" . $i])) {
						...
						$photo = $func->uploadImage("file" . $i, $config['photo']['man_photo'][$type]['img_type_photo'], UPLOAD_PHOTO, $file_name . $i);
						if ($photo) {
