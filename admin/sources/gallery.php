	...
	if ($id) {
		...
		if ($numberPhoto && $dataMultiTemp) {
			...
			for ($i = 0; $i < $numberPhoto; $i++) {
				...
				if (isset($_GET['val']) && $_GET['val'] == 'taptin') {
			...
			if (isset($flag_fail) && isset($flag_success)) {
				if (($flag_fail == count($dataMultiTemp))) {
					$func->transfer("Lưu dữ liệu thất bại1", "index.php?com=" . $com . "&act=man_photo&idc=" . $idc . "&type=" . $type . "&kind=" . $kind . "&val=" . $val . "&p=" . $curPage, false);
