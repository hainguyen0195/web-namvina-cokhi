            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <?php } ?>
                        <?php if (isset($config['product'][$type]['show_images_list2']) && $config['product'][$type]['show_images_list2'] == true) { ?>
                            <th class="align-middle">Banner 2</th>
                        <?php } ?>
                        <?php if (isset($config['product'][$type]['show_icon_list']) && $config['product'][$type]['show_icon_list'] == true) { ?>
                            <th class="align-middle">Icon</th>
                        <?php } ?>
                        <?php if (isset($config['product'][$type]['show_icon_list2']) && $config['product'][$type]['show_icon_list2'] == true) { ?>
                            <th class="align-middle">Icon 2</th>
                        <?php } ?>
                    ...
                    <tbody>
                        <?php for ($i = 0; $i < count($items); $i++) { ?>
                            <tr>
                                ...
                                <?php if (isset($config['product'][$type]['show_images_list2']) && $config['product'][$type]['show_images_list2'] == true) { ?>
                                    <td class="align-middle">
                                        <a href="<?= $linkEdit ?>&id=<?= $items[$i]['id'] ?>" title="<?= $items[$i]['tenvi'] ?>"><img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?= THUMBS ?>/<?= $config['product'][$type]['thumb_list'] ?>/<?= UPLOAD_PRODUCT_L . $items[$i]['photo2'] ?>" alt="<?= $items[$i]['tenvi'] ?>"></a>
                                    </td>
                                <?php } ?>
                                <?php if (isset($config['product'][$type]['show_icon_list']) && $config['product'][$type]['show_icon_list'] == true) { ?>
                                    <td class="align-middle">
                                        <a href="<?= $linkEdit ?>&id=<?= $items[$i]['id'] ?>" title="<?= $items[$i]['tenvi'] ?>"><img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?= THUMBS ?>/<?= $config['product'][$type]['thumb_list'] ?>/<?= UPLOAD_PRODUCT_L . $items[$i]['icon'] ?>" alt="<?= $items[$i]['tenvi'] ?>"></a>
                                    </td>
                                <?php } ?>
                                <?php if (isset($config['product'][$type]['show_icon_list2']) && $config['product'][$type]['show_icon_list2'] == true) { ?>
                                    <td class="align-middle">
                                        <a href="<?= $linkEdit ?>&id=<?= $items[$i]['id'] ?>" title="<?= $items[$i]['tenvi'] ?>"><img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?= THUMBS ?>/<?= $config['product'][$type]['thumb_list'] ?>/<?= UPLOAD_PRODUCT_L . $items[$i]['icon2'] ?>" alt="<?= $items[$i]['tenvi'] ?>"></a>
                                    </td>
                                <?php } ?>
                                <td class="align-middle">
