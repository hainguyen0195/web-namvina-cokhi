...
?>
		...
		<?= ($giakm > 0 && $giakm < 100) ? '<div class="main-discount">(' . tietkiem . ' ' . $this->format_money($giacu - $giamoi) . ' ~ ' . $giakm . '%)</div>' : '' ?>
	...
	public function showProductRow1($array)
	{
		global $lang, $sluglang;
	?>
		<?php foreach ($array as $k => $v) {
			$setupitem = (isset($v['info_more']) && $v['info_more'] != '') ? json_decode($v['info_more'], true) : null;
		?>
			<div class="pro1-item wow fadeInUp" data-wow-duration="<?= ($k + 1) / 10 + 0.2 ?>s" data-wow-delay="<?= ($k + 1) / 10 + 0.2 ?>s">
				<div class="pro1-left" style="background: <?= ($setupitem['background']) ? '#' . $setupitem['background'] : '#F5F5F5' ?>;">
					<div class="img_pro1 img_">
						<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>">
							<img onerror="this.src='<?= THUMBS ?>/280x230x1/assets/images/noimage.png';" src="<?= THUMBS ?>/280x230x1/<?= UPLOAD_PRODUCT_L . $v['photo2'] ?>" alt="<?= $v['ten' . $lang] ?>" />
						</a>
					</div>
					<h3 class="name_pro1">
						<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>" style="color: <?= ($setupitem['colortext1']) ? '#' . $setupitem['colortext1'] : 'var(--colormainhv)' ?>;">
							<?= $v['ten' . $lang] ?>
						</a>
					</h3>
					<div class="des_pro1" style="color: <?= ($setupitem['colortext1']) ? '#' . $setupitem['colortext1'] : '#000000' ?>;">
						<?= $v['mota' . $lang] ?>
					</div>
					<div class="timhieuthem">
						<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>" style="color: <?= ($setupitem['colortext1']) ? '#' . $setupitem['colortext1'] : '#000000' ?>;">
							Tìm hiểu thêm <i class="fal fa-long-arrow-right"></i>
						</a>
					</div>
				</div>
				<div class="pro1-right">
					<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>">
						<img onerror="this.src='<?= THUMBS ?>/1060x610x1/assets/images/noimage.png';" src="<?= THUMBS ?>/1060x610x1/<?= UPLOAD_PRODUCT_L . $v['photo'] ?>" alt="<?= $v['ten' . $lang] ?>" />
					</a>
				</div>
			</div>
		<?php } ?>
	...
	public function showProductRow($array, $row = 'row', $col = 'col-md-3 col-sm-4 col-xs-6')
	...
	?>
		<div class="<?= $row ?>">
			<?php foreach ($array as $k => $v) { ?>
				<div class="<?= $col ?> wow fadeInUp" data-wow-duration="<?= ($k + 1) / 10 + 0.4 ?>s" data-wow-delay="<?= ($k + 1) / 10 + 0.4 ?>s">
					<div class="pro">
						<div class="img_pro img_">
							<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>">
								<img onerror="this.src='<?= THUMBS ?>/280x320x1/assets/images/noimage.png';" src="<?= THUMBS ?>/280x320x1/<?= UPLOAD_PRODUCT_L . $v['photo'] ?>" alt="<?= $v['ten' . $lang] ?>" />
							</a>
						</div>
						<h3 class="name_pro">
							<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>">
	...
	public function showServiceRow($array, $row = 'row', $col = 'col-md-4 col-sm-6 col-xs-12')
	{
		global $lang, $sluglang;
	?>
		<div class="<?= $row ?>">
			<?php foreach ($array as $k => $v) { ?>
				<div class="<?= $col ?> wow fadeInUp" data-wow-duration="<?= ($k + 1) / 10 + 0.4 ?>s" data-wow-delay="<?= ($k + 1) / 10 + 0.4 ?>s">
					<div class="dichvu-item">
						<div class="dichvu-item-img img_">
							<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>">
								<img onerror="this.src='<?= THUMBS ?>/680x440x1/assets/images/noimage.png';" src="<?= THUMBS ?>/680x440x1/<?= UPLOAD_NEWS_L . $v['photo'] ?>" alt="<?= $v['ten' . $lang] ?>" />
							</a>
						</div>
						<div class="dichvu-item-content">
							<h3 class="dichvu-item-name">
								<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>">
									<?= $v['ten' . $lang] ?>
								</a>
							</h3>
							<div class="dichvu-item-des">
								<?= $v['mota' . $lang] ?>
							</div>
							<div class="dichvu-item-link-detail">
								<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>">
									<?= timhieuthem ?> >>
								</a>
							</div>
	...
	public function showPostRow($array, $row = 'row', $col = 'col-md-6 col-sm-6 col-xs-12')
	{
		global $lang, $sluglang;
	?>
		<div class="<?= $row ?>">
			<?php foreach ($array as $k => $v) { ?>
				<div class="<?= $col ?> wow fadeInUp" data-wow-duration="<?= ($k + 1) / 10 + 0.4 ?>s" data-wow-delay="<?= ($k + 1) / 10 + 0.4 ?>s">
					<div class="post-item flex_row">
						<div class="post-item-img img_">
							<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>">
								<img onerror="this.src='<?= THUMBS ?>/280x230x1/assets/images/noimage.png';" src="<?= THUMBS ?>/280x230x1/<?= UPLOAD_NEWS_L . $v['photo'] ?>" alt="<?= $v['ten' . $lang] ?>" />
							</a>
						</div>
	...
	?>
		<div class="<?= $col ?>">
			<div class="pro">
				<div class="img_pro img_">
					<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>">
						<img onerror="this.src='<?= THUMBS ?>/280x230x1/assets/images/noimage.png';" src="<?= THUMBS ?>/280x230x1/<?= UPLOAD_PRODUCT_L . $v['photo'] ?>" alt="<?= $v['ten' . $lang] ?>" />
					</a>
		...
		<div class="<?= $col ?>">
			<div class="box_news flex_row">
				<div class="img_news img_">
					<a href="<?= $v[$sluglang] ?>" title="<?= $v['ten' . $lang] ?>">
						<img onerror="this.src='<?= THUMBS ?>/280x220x1/assets/images/noimage.png';" src="<?= THUMBS ?>/280x220x1/<?= UPLOAD_NEWS_L . $v['photo'] ?>" alt="<?= $v['ten' . $lang] ?>" />
					</a>
				</div>
...
<?php
	...
	public function get_infoproduct($id = 0, $element = '', $table = '', $type = '')
	{
		if ($id) {
			$temps = $this->d->rawQueryOne("select id_info from #_" . $table . " where id = ? and type = ? limit 0,1", array($id, $type));
			$arr_info = explode(',', $temps['id_info']);

			for ($i = 0; $i < count($arr_info); $i++) $temp[$i] = $arr_info[$i];
		}

		$row_infokey = $this->d->rawQuery("select tenvi, id from #_product_list where type = ? and hienthi > 0 order by stt,id desc", array('thong-so'));

		$str = '<select id="' . $element . '" name="' . $element . '[]" class="select multiselect thongsosanpham" multiple="multiple" >';
		for ($i = 0; $i < count($row_infokey); $i++) {
			if (isset($temp) && count($temp) > 0) {
				if (in_array($row_infokey[$i]['id'], $temp)) $selected = 'selected="selected"';
				else $selected = '';
			} else {
				$selected = '';
			}
			$str .= '<option value="' . $row_infokey[$i]["id"] . '" ' . $selected . ' /> ' . $row_infokey[$i]["tenvi"] . '</option>';
		}
		$str .= '</select>';
	...
	function createContentMail($contentChinh, $sub, $title)
	{
		global $emailer;
		$content = '
												...
												<tbody>
													<tr>
														<td>
															...
															<p style="margin:4px 0;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#444;line-height:18px;font-weight:normal">' . $sub . ' ' . $emailer->getEmail('company:website') . '.</p>
														</td>
													</tr>
													<tr>
														<td style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#444;line-height:18px">
															<table border="0" cellpadding="0" cellspacing="0" width="100%">
																<thead>
																	<tr>
																		<th align="left" style="padding:6px 9px 0px 0px;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#444;font-weight:bold" width="50%">
																			<h3 style="font-size:13px;font-weight:bold;color:' . $emailer->getEmail('color') . ';text-transform:uppercase;margin:20px 0 0 0;padding: 0 0 5px;border-bottom:1px solid #ddd">' . $title . ' <span style="font-size:12px;color:#777;text-transform:none;font-weight:normal">(' . ngay . ' ' . date('d', $emailer->getEmail('datesend')) . ' ' . thang . ' ' . date('m', $emailer->getEmail('datesend')) . ' ' . nam5 . ' ' . date('Y H:i:s', $emailer->getEmail('datesend')) . ')</span></h3>
																		</th>
																	</tr>
																</thead>
																<tbody>
													...
													<tr>
														<td>&nbsp;
															<p style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#444;line-height:18px;font-weight:normal;border:1px ' . $emailer->getEmail('color') . ' dashed;padding:10px;list-style-type:none">' . bancanhotrongay . ' mail: <a href="mailto:' . $emailer->getEmail('company:email') . '" style="color:' . $emailer->getEmail('color') . ';text-decoration:none" target="_blank"> <strong>' . $emailer->getEmail('company:email') . '</strong> </a> - Hotline:<span style="color:' . $emailer->getEmail('color') . ';">' . $emailer->getEmail('company:hotline') . '</span></p>
														</td>
													</tr>
													<tr>
														<td>&nbsp;
						...
						<td align="center">
							<table width="600">
								<tbody>
									<tr>
										<td>
											<p align="left" style="font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:18px;color:#4b8da5;padding:10px 0;margin:0px;font-weight:normal">' . quykhachdanhandcmailvilienhe . ' ' . $emailer->getEmail('company:website') . '.<br>
											' . dechachchannhanemailthongbao . ' ' . $emailer->getEmail('company:website') . ', ' . quykhachvuilongthemmail . ' <strong><a href="mailto:' . $emailer->getEmail('email') . '" target="_blank">' . $emailer->getEmail('email') . '</a></strong> ' . vaosodiachicuahopmail . '.<br>
										</td>
									</tr>
								</tbody>
							</table>
